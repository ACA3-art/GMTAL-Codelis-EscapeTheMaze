...
public class MenuManager : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        {
            backgroundSlider.value = PlayerPrefs.GetFloat("BackGroundSound", 0.75f);
        }

        if(characterSlider != null)
        {
            characterSlider.value = PlayerPrefs.GetFloat("CharacterSound", 0.75f);
        }
        if(ShowFPS != null)
        {
            ShowFPS.isOn = PlayerPrefs.GetInt("ShowFPS", 1) == 1;
        }
    ...
    public void SetBackGroundVolumeSound(float slideValue)
    {
        PlayerPrefs.SetFloat("BackGroundSound", slideValue);
        PlayerPrefs.Save();
        BackGroundSound.Instance.source.volume = slideValue;
    }
    public void SetCharacterVolumeSound(float slideValue)
    {
        PlayerPrefs.SetFloat("CharacterSound",slideValue);
        PlayerPrefs.Save();
    }
